...
class Advanced extends Customize {
	...
	public function css($is_frontend) {
		...
		if(is_array($advanced)) {
			...
			if(isset($advanced['text_color'])) {
				$css .= '.is-content { color: ' . $advanced['text_color'] . '; }';
			}
			// text color
			if(isset($advanced['link_color'])) {
				$css .= '.is-content a { color: ' . $advanced['link_color'] . '; }';
			}
			// text color
			if(isset($advanced['mouseover_color'])) {
				$css .= '.is-content a:hover { color: ' . $advanced['mouseover_color'] . '; }';
			}
			// global custom css
			if($is_frontend) {
				// progress bar
				if(isset($advanced['progress_bar'])) {
					$css .= '#nprogress .bar { background: ' . $advanced['progress_bar'] . '; }';
				}
				// progress bar height
				if(isset($advanced['progress_bar_height'])) {
					$css .= '#nprogress .bar { height: ' . $advanced['progress_bar_height'] . '; }';
				}	
				// top arrow visibility
				if(isset($advanced['top_arrow_visibility']) && $advanced['top_arrow_visibility'] == 'hidden') {
					$css .= '.back-to-top { display: none !important; }';
				}
				// top arrow color
				if(isset($advanced['top_arrow_color'])) {
					$css .= '.back-to-top a svg { fill: ' . $advanced['top_arrow_color'] . '; }';
				}
				// top arrow width
				if(isset($advanced['top_arrow_width'])) {
					$css .= '.back-to-top a img, .back-to-top a svg { width: ' . $advanced['top_arrow_width'] . '; height: auto; }';
				}
				// password font color
				if(isset($advanced['password_color'])) {
					$css .= '.post-password-form p, .post-password-form p a.post-password-submit, .post-password-form input[type=submit] { color: ' . $advanced['password_color'] . '; }';
				}
				// password border color
				if(isset($advanced['password_border'])) {
					$css .= '.post-password-form p a.post-password-submit, .post-password-form p input[type=submit], .post-password-form p label input { border-color: ' . $advanced['password_border'] . '; }';
				}
				// lightbox bg color
				if(isset($advanced['lightbox_bg_color'])) {
					$css .= '.pswp__bg { background: ' . $advanced['lightbox_bg_color'] . '; }';
					$bg_color = Color::hex_to_rgb($advanced['lightbox_bg_color']);
					// transparent top bar
					$css .= '.pswp__top-bar, .pswp__button--arrow--left:before, .pswp__button--arrow--right:before { background-color: rgba(' . $bg_color['r'] . ', ' . $bg_color['g'] . ', ' . $bg_color['b'] . ', .15) !important; }';
				}
				// lightbox opacity
				if(isset($advanced['lightbox_bg_opacity'])) {
					$css .= '.pswp__bg { opacity: ' . $advanced['lightbox_bg_opacity'] . ' !important; }';
				}
				// lightbox icon opacity
				if(isset($advanced['lightbox_icon_opacity'])) {
					$css .= '.pswp__button, .pswp__counter { opacity: ' . $advanced['lightbox_icon_opacity'] . ';}';
				}
				// lightbox font familry
				if(isset($advanced['lightbox_font'])) {
					$css .= '.pswp__caption__center, .pswp__counter {' . Typography::get_font_family($advanced['lightbox_font']) . '}';
				}
				// lightbox font size
				if(isset($advanced['lightbox_fontsize'])) {
					$css .= '.pswp__caption__center { font-size: ' . $advanced['lightbox_fontsize'] . ';}';
				}
				// lightbox caption spacing
				if(isset($advanced['lightbox_spacing'])) {
					$css .= '.pswp__caption__center { margin-bottom: ' . $advanced['lightbox_spacing'] . ';}';
				}
				// lightbox icon color
	...
	public function user_css() {
		...
		if(is_array($advanced)) {
			if(isset($advanced['custom_css_global']) && !empty($advanced['custom_css_global'])) {
				$css .= $advanced['custom_css_global'];
			}
			// get breakpoints
			$breakpoints = array(
			...
			foreach ($breakpoints as $breakpoint => $query) {
				if(isset($advanced['custom_css_' . $breakpoint]) && !empty($advanced['custom_css_' . $breakpoint])) {
					$css .= $query . '{' . $advanced['custom_css_' . $breakpoint] . '}';
				}
			}
		}
		// output
...
Customize::$setting['advanced'] = new Advanced;